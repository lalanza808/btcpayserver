...
namespace BTCPayServer.Services.Altcoins.Wownero.Payments
{
    public class WowneroCheckoutModelExtension : ICheckoutModelExtension
    {
        ...
        {
            if (context is not { Handler: WowneroLikePaymentMethodHandler handler })
                return;
            ...
            var details = context.InvoiceEntity.GetPayments(true)
                    .Select(p => p.GetDetails<WowneroLikePaymentData>(handler))
                    .Where(p => p is not null)
                    .FirstOrDefault();
            if (details is not null)
            {
                context.Model.ReceivedConfirmations = details.ConfirmationCount;
                context.Model.RequiredConfirmations = (int)WowneroListener.ConfirmationsRequired(details, context.InvoiceEntity.SpeedPolicy);
            }
